...
class PurchaseReturnOfflineCubit extends Cubit<List<dynamic>> {
  PurchaseReturnOfflineCubit() : super([]) {
    loadData();
  }

  final Box box = Hive.box('purchase_return');
  ...
  Future<void> post(PurchaseReturnFailedOfflineCubit failCubit) async {
    final items = getJsonData();
    if (items.isEmpty) {
