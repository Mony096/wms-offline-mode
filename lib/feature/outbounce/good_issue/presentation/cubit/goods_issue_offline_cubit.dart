...
class GoodsIssueOfflineCubit extends Cubit<List<dynamic>> {
  GoodsIssueOfflineCubit() : super([]) {
    loadData();
  }

  final Box box = Hive.box('goods_issue');
...
// 🔹 Remove record by failId
  ...
  Future<void> post(GoodIssueFailedOfflineCubit failCubit) async {
    final items = getJsonData();
    if (items.isEmpty) {
