...
import 'package:wms_mobile/utilies/storage/locale_storage.dart';
import '/constant/style.dart';
import '../cubit/batch_list_offline_cubit.dart';
...
class _BatchListPageState extends State<BatchListPage> {
  ...
  void init(BuildContext context) async {
    ...
    // 🔹 Filter by itemCode, binCode, and warehouse
    final filtered = offlineData.where((e) {
      final matchesItem = e['ItemCode'] == widget.itemCode;
      final matchesWarehouse = e['WhsCode'] == warehouse;

      // ✅ Safely convert widget.binCode to int for comparison
      final binCodeInt = int.tryParse(widget.binCode.toString());

      final matchesBin =
          widget.binCode != "" ? e['AbsEntry'] == binCodeInt : true;
