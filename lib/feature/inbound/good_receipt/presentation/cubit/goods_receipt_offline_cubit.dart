...
class GoodsReceiptOfflineCubit extends Cubit<List<dynamic>> {
  GoodsReceiptOfflineCubit() : super([]) {
    loadData();
  }

  final Box box = Hive.box('goods_receipt');
...
// 🔹 Remove record by failId
  ...
  Future<void> post(GoodReceiptFailedOfflineCubit failCubit) async {
    final items = getJsonData();
    if (items.isEmpty) {
