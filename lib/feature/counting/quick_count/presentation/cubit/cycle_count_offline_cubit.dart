...
class CycleCountOfflineCubit extends Cubit<List<dynamic>> {
  CycleCountOfflineCubit() : super([]) {
    loadData();
  }

  final Box box = Hive.box('cycle_count');
...
// 🔹 Remove record by failId
  ...
  Future<void> post(CycleCountFailedOfflineCubit failCubit) async {
    final items = getJsonData();
    if (items.isEmpty) {
      print("⚠️ No offline records to sync.");
      return;
